...
if (isset($_GET['act'])) {
	if ($_GET['act'] == 'cart') {
		if (isset($_GET['id']) and is_numeric($_GET["qty"]) and $_GET["qty"]<>0) {
			...
			if ($results) {
				$new_transaksi = array(array('service_id' => $obj->id, 'service_name' => $obj->name, 'service_size' => $obj->size,
					'service_price' => $obj->price, 'service_qty' => $_GET["qty"]));
				if (isset($_SESSION['transaksi'])) {
					...
					if ($found == false) {
						...
						echo "Service berhasil ditambahkan dengan jumlah ".$_GET["qty"];
					} else {
