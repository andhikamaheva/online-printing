<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-custom">
	<thead>
		<tr>
			...
			<th><label><span class="sr-only">Tanggal Tolak</span><input type="text" name="search_votes" value="Cari Tanggal Tolak" class="search_init" /></label></th>
	...
	<?php
		...
		$query="SELECT t.transaksi_open, t.transaksi_close, m.member_name, a.admin_name FROM transaksi t left join member m on t.member_member_username=m.member_username left join admin a on t.admin_admin_username=a.admin_username WHERE t.transaksi_payment is not null and t.transaksi_approve is null and t.transaksi_close is not null";
		$nom=0;
